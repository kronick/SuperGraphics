...
void ofxSVG::load(string svgPath){
    ...
    if (!file && bVerbose){
        ...
        if(svgString.find("<svg") == -1 && bVerbose){
            ...
            // -------------------------------------
            int afterSvgTagPos = svgString.find(">");
            int svgCloseTag = svgString.find("</svg>");
            string start = svgString.substr(0,afterSvgTagPos+1);
            string middle = svgString.substr(afterSvgTagPos+1, svgString.length() - afterSvgTagPos+1 + svgCloseTag);
            svgString = start + "<g>" + middle + "</g>" + "</svg>";
...
ofVec2f ofxSVG::getSize() {
    return ofVec2f(docWidth, docHeight);
}
...
void ofxSVG::parseLayer(){
    ...
    for(int i=0; i<numObjects; i++) {
        ...
        else if(name == "polyline") parsePolyline();
...
void ofxSVG::parsePolygon(){
    ...
    layers[layers.size()-1].objects.push_back(obj);
    */
    string pathStr = svgXml.getAttribute("points", currentIteration);
    pathStr = "M" + pathStr;    // Move into position for first point
...
vector<ofxSVGObject*> ofxSVG::getObjects() {
    vector<ofxSVGObject*> objects;
    for(int i=0; i<layers.size(); i++) {
        for(int j=0; j<layers[i].objects.size(); j++) {
            objects.push_back(layers[i].objects[j]);
        }
     }
