...
public:
    ...
    void setOpacity(float o) {
        opacity = o;
        strokeColor = ofColor(strokeColor, o);
        fillColor = ofColor(fillColor, o);
        render();
    }
...
public:
	...
	void draw() {
		...
		if(fill && antialiased) {  // Draw stroke too, for antialiasing
            path.setFilled(false);
            path.setStrokeWidth(1);
            
            path.setStrokeColor(ofColor(fillColor, opacity));
            path.draw();
            render();   // resets style
		}
	}
	...
	void render() {
		...
		path.setStrokeColor(ofColor(strokeColor, opacity));
		path.setFilled(fill);
		path.setFillColor(ofColor(fillColor, opacity));
	}
